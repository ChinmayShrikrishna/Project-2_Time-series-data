...
stock_data = new.env()
stock_list = c('PNB.NS')
start_date = as.Date('2015-01-01'); end_date = as.Date('2024-03-02')
getSymbols(Symbols = stock_list, from = start_date, to = end_date, env = stock_data)
stock_price=na.omit(stock_data$PNB.NS$PNB.NS.Adjusted)
#pnb_price
...
```{r}
# Augmented Dickey-Fuller (ADF) Test for Stationarity with PNB Data
# *******************************************************************

adf_test_pnb = adf.test(stock_price);adf_test_pnb
# Inference : PNB Time-Series is Non Stationary

```

```{r}
pnb_ds = diff(log(stock_price)); plot(pnb_ds) # PNB (First)return Difference Time-Series
```

```{r}
pnb_ds=na.omit(pnb_ds)
adf_test_pnb_ds = adf.test(pnb_ds); adf_test_pnb_ds # Inference : PNB Difference Time-Series is Stationary

```

```{r}
# Ljung-Box Test for Autocorrelation - PNB Data
# ***********************************************

lb_test_pnb_ds = Box.test(pnb_ds); lb_test_pnb_ds # Inference : PNB Difference (Stationary) Time-Series is Autocorrelated as NULL is rejected and p-value<0.0151 | NULL: No Auto correlation | Alternate: Auto Correlation
```
...
acf(pnb_ds) # ACF of PNB Difference (Stationary) Series
pacf(pnb_ds) # PACF of PNB Difference (Stationary) Series
```
...
```{r}
arma_pq_pnb_ds = auto.arima(pnb_ds); arma_pq_pnb_ds #p-lag=2, q-lag=2
```

```{r}
pnb_ds_fpq = forecast(arma_pq_pnb_ds, h = 500)
plot(pnb_ds_fpq)
```
...
lb_test_arma_pq_pnb_ds = Box.test(arma_pq_pnb_ds$residuals); lb_test_arma_pq_pnb_ds
#p-value>alpha
```
...
pnb_ret_sq = arma_pq_pnb_ds$residuals^2 # Residual Variance (Since Mean Returns is approx. 0)
plot(pnb_ret_sq)
pnb_ret_sq_box_test = Box.test(pnb_ret_sq, lag = 2) # H0: Return Variance Series is Not Serially Correlated
pnb_ret_sq_box_test # Inference : Return Variance Series is Autocorrelated (Has Volatility Clustering)
```

```{r}
# Test for Volatility Clustering or Heteroskedasticity: ARCH Test
pnb_ret_arch_test = ArchTest(arma_pq_pnb_ds$residuals^2, lags = 2) # H0: No ARCH Effects
pnb_ret_arch_test # Inference : Return Series is Heteroskedastic (Has Volatility Clustering)
```
...
pnb_ret_garch1 = ugarchfit(garch_model1, data = arma_pq_pnb_ds$residuals^2); pnb_ret_garch1

```

```{r}
# Test for Volatility Clustering or Heteroskedasticity: ARCH Test
pnb_garch_arch_test = ArchTest(residuals(pnb_ret_garch1)^2, lags = 1) # H0: No ARCH Effects
pnb_garch_arch_test # Inference : Return Series is Heteroskedastic (Has Volatility Clustering)
#pnb_ret_garch1
```
...
pnb_ret_garch2 = ugarchfit(garch_model2, data = pnb_ds); pnb_ret_garch2

# GARCH Forecast
pnb_ret_garch_forecast1 = ugarchforecast(pnb_ret_garch1, n.ahead = 500); pnb_ret_garch_forecast1
pnb_ret_garch_forecast2 = ugarchforecast(pnb_ret_garch2, n.ahead = 500); pnb_ret_garch_forecast2
```

```{r}
plot(pnb_ret_garch_forecast2)
```
